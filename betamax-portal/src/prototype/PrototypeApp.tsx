...
const Dashboard = () => {
    ...
    const myFeedback = React.useMemo(() => feedback.filter(f => f.reporterId === user?.id), [feedback, user?.id]);
...
const NavItem = React.memo(({ path, icon, label, isActive, navigate }: { path: string; icon: string; label: string; isActive: boolean; navigate: (path: string) => void }) => (
    <button
        ...
        <>
            ...
            <div className="fixed bottom-0 left-0 right-0 z-40 px-6 pb-6 pt-4 bg-gradient-to-t from-background via-background to-transparent pointer-events-none">
                <nav className="pointer-events-auto bg-[#18181b]/90 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl flex justify-between items-center px-6 py-3 max-w-md mx-auto">
                    <NavItem path="/" icon="dashboard" label="Home" isActive={isActive('/')} navigate={navigate} />
                    ...
                    <NavItem path="/profile" icon="person" label="Profile" isActive={isActive('/profile')} navigate={navigate} />
                </nav>
            </div>
        </>
    );
