...
const DEFAULT_STATUS_STYLE = "bg-gray-800 text-gray-400 border-gray-700";

const StatusBadge = ({ status, type = 'status' }: { status: string, type?: 'status' | 'severity' }) => {
    return (
        <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold uppercase border ${STATUS_STYLES[status] || DEFAULT_STATUS_STYLE}`}>
            {status}
        </span>
    );
};
