...
**Learning:** Found multiple instances of icon-only `motion.button` elements (using `lucide-react` icons) lacking `aria-label` or tooltips, making them inaccessible to screen readers and unclear to mouse users.
**Action:** Audit all icon-only buttons in the main application flow and enforce `aria-label` and `title` attributes to ensure both accessibility and usability.
